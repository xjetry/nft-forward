...
设计取向(用户明确要求):**最少技术债 / 最长期正确 / 最完整**,而非最小改动。
因此本设计引入一层一等公民的数据平面抽象,而不是把中继当作 applier 的旁挂。
...
`Dataplane` 持有上次成功落地的 `lastKernel` / `lastUserspace`。
`Reconcile(ctx, rules)`:
...
3. `kernel.Reconcile(kernelRules)`:nft 原子应用 + tc。**硬失败**(nft 失败)
   → 回滚用户态到 `lastUserspace`(`userspace.Reconcile(lastUserspace)`),
   返回错误。nft 原子:失败则内核保持旧表(与回滚后的用户态一致)。
4. `firewall.Sync(...)`:**尽力而为**,失败仅记日志不回滚(沿用现有
   「shim 失败非致命」语义)。
5. 全部成功 → 更新 `lastKernel`/`lastUserspace`。
...
- 期望集 D(tcp 用户态规则,按 SrcPort)对当前集 C:
  ...
  - C 有 D 无 → 关监听器(停 accept;在途连接在 `Close(ctx)` 带超时强制收口)。
...
- **不用 splice/io.Copy**:刻意放弃零拷贝以换取(a)连接存活期间的**连续**
  ...
  (b)限速/不限速代码统一(limiter 为 nil 即不限速)。见 §14 取舍。
...
`Close(ctx)`:停所有 accept,给在途连接一个 ctx 截止期收口,到期强制关。
由 Dataplane.Close 调用,纳入 daemon `Run` 关停的 5s 预算
(与 dialer 停机、`srv.Shutdown` 顺序串行,见 daemon.go)。
...
- **`forward/userspace` relay**:loopback 端到端拷贝正确;半关传播;令牌桶限速
  ...
  目标);带宽热更新;`Close(ctx)` 优雅收口 + 超时强制关;bind 冲突 →
  make-before-break 回退。
- **`forward.Dataplane`**:kernel 失败回滚 userspace;Partition 失败不触碰后端;
  firewall 失败非致命;`lastKernel/lastUserspace` 维护;Counters 合并。
...
- **放弃 splice/零拷贝**(用户态):换连续计数 + 限速/不限速代码统一。
  用户态价值在 cwnd 隔离(PEP),非吞吐峰值;裸 splice 系统调用代码反而是
  更多技术债。中继节点带宽量级下缓冲拷贝 CPU 成本可接受。
