...
- 链式规则每一跳按**该跳节点上的 grant** 限速（与配额挂靠节点的语义一致），端到端速度为各跳最小值。
...
- `nft.Rule` 新增数据面字段 `OwnerID int64`（json `owner_id,omitempty`）与 `RateMBytes int`（json `rate_mbytes,omitempty`）。旧 agent 解 JSON 忽略未知字段，天然兼容。
- `buildRules` 按 (owner, 该跳节点) 查 grant 填这两个字段；同时把线上的旧字段 `bandwidth_mbps` 填为等效 Mbit 值（≈ rate × 8.389），让**未升级的旧 agent 以"每规则各限 X"降级执行**，升级后自动变为共享总桶。
- 字段优先级不变量：agent 侧凡 `owner_id`+`rate_mbytes` 存在即走分组限速并忽略 `bandwidth_mbps`；仅有 `bandwidth_mbps`（旧 server）时保留现有每规则限速路径。同一份下发内两种形态不会混出现（新 server 只在有 grant 时才填限速，两套字段同源）。
- `computeRev` 继续只剥离 RuleID/RuleName/OwnerName 三个展示字段；`owner_id`/`rate_mbytes` 参与 rev 哈希，保证改限速能触发重下发。首次升级会因新字段引起一次性全量重推，无害。
- standalone 降级路径（面板脱管清理）把 `owner_id`/`rate_mbytes` 随面板元数据一并清空——分组限速是面板驱动的策略，脱管后不应残留。
...
- 新增按 `owner_id` 键控、跨 Reconcile 稳定的共享 `*rate.Limiter` 注册表；限速值变化用 `SetLimit` 热更而非重建，避免清空桶状态。组内所有 listener 共用同一 limiter。
...
- 首包：prerouting DNAT 处对属于限速组的规则执行 `ct mark set (0x10000 | owner_id)`——偏移使组 mark 与旧式端口 mark（≤ 0xFFFF）空间不相交，混合过渡期互不误伤。
- 每包：新增 filter 类 prerouting 链，`ct mark != 0` 时 `meta mark set ct mark` 恢复——转发流量双向都经 prerouting，一条恢复规则覆盖两个方向。
- tc：每组一个 HTB class（`classid 1:<owner_id>`，rate=ceil=X MB/s，以 bit 精确换算 X × 8388608 bit/s），fw filter 按 mark（`0x10000 | owner_id`）分类到该 class。不再有按端口的 class。
- `owner_id > 0xFFFF` 时该组跳过 kernel 整形并打 WARN（tc class minor 仅 16 位；用户 id 实际远小于此）。
- connmark 跨 re-apply 持久：组的 mark 值直接取自稳定的 `owner_id` 而非任何本地序号分配，重下发前后同一连接的分类保持正确。
