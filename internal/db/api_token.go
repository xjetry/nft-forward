...
func CreateAPIToken(d *sql.DB, userID int64) (string, error) {
	token := RandToken(32)
	_, err := d.Exec(
		`INSERT INTO api_tokens(user_id, token, token_prefix, created_at) VALUES (?,?,?,?)`,
...
func GetAPITokenByUser(d *sql.DB, userID int64) (*APIToken, error) {
	...
	var disabled int
	err := d.QueryRow(
		`SELECT id, user_id, token, token_prefix, disabled, created_at, last_used_at FROM api_tokens WHERE user_id=?`,
