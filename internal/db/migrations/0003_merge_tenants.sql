-- Merge tenants into users: tenant quota/traffic/expiry fields move onto the
-- user row; forwards and chains reference owner_id (users.id) instead of
-- tenant_id (tenants.id); grant tables become user_tunnels/user_tunnel_combos.
...
-- 7. Drop old tables.
DROP TABLE IF EXISTS tenant_tunnels;
DROP TABLE IF EXISTS tenant_tunnel_combos;
