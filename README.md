...
**生命周期与一致性**：链路对底层拓扑变化是自愈的。修改节点「中继地址」或删除链路中途的
节点时，系统自动重算受影响的链路并重新对齐下发，避免上一跳继续打向旧 / 失效地址造成静默
断流。对租户链路，若删节点使链路缩短、且新的末跳通道目标 CIDR 不再允许租户选定的出口，
该链路会被整条拆除，而不是把流量导向越权目标。仍被任意链路或转发占用的通道不可删除——
删除请求会被拒绝并提示占用条数，需先解除这些占用。
...
依赖：Go ≥ 1.26，无 CGO，跨平台编译。运行时依赖 `nftables` + `iproute2`（仅 Linux）。
